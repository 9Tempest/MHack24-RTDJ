...
sys.path.append("C:\\Users\\intelaipc\\Downloads\\MHack24-RTDJ")
...
def encoder_poses(frame, unpreprocessed: np.ndarray):
    """
    ...
    results = compiled_model([unpreprocessed])

    pafs = results[pafs_output_key]
    heatmaps = results[heatmaps_output_key]
    # Get poses from network results.
    poses, scores = process_results(frame, pafs, heatmaps)
    encoded_poses = poses / 100
    if encoded_poses.shape[0] >= 3:
        encoded_poses = encoded_poses[0:3][:][:]
    else:
        for i in range(3-encoded_poses.shape[0]):
            encoded_poses = np.concatenate((encoded_poses, np.zeros((1,17,3))))
